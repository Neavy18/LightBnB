...
const getUserWithEmail = function(email) {
  return pool.query(`
    ...
    .catch((err) => {
      console.log(err.message, "---> ERROR in getUserWithEmail <--- ");
    });
};
...
const getUserWithId = function(id) {
  return pool.query(`
    ...
    .catch((err) => {
      console.log(err.message, "---> ERROR in getUserWithId<--- ");
    });
};
...
const addUser = function(user) {
  ...
  VALUES ($1, $2, $3)`, [user.name, user.password, user.email])
    ...
    .catch((err) => {
      console.log(err.message, "---> ERROR in addUSer <--- ");
    });
};
...
const getAllReservations = function(guest_id, limit = 10) {
  ...
  LIMIT $2;`,[guest_id, limit])
    ...
    .catch((err) => {
      console.log(err.message, "---> ERROR IN getAllREservations <--- ");
    });
};
